//FP3211.v memory-mapped single-shader implementation with coarse-grained scheduler (CGS)
`timescale 1ns/100ps
// SYMPL 32-Bit Multi-Thread, Multi-Processing GP-GPU-Compute Engine (single shader)
// Author:  Jerry D. Harthcock
// Version:  1.02  December 12, 2015
// October 30, 2015
...
assign DMA1collide = DMA1_datapool_collide | DMA1_PDB_collide0;
assign DMA0collide = DMA0_datapool_collide | DMA0_PDB_collide0 | DMA0_rom_collide | DMA0_rom0_collide;
...
assign cgs_rom0_wren  = cgs_wrcycl & (cgs_dest_q2[23:12]==12'h018);

assign cgs_rom_wren   = cgs_wrcycl & (cgs_dest_q2[23:12]==12'h01D);
assign DMA0_rom_wren = DMA0wren & (DMA0wraddrs[23:12]==12'h01D);

assign DMA0_rom0_wren = DMA0wren & (DMA0wraddrs[23:12]==12'h018); 

assign done_thread[15:4] = 12'h000;
...
CGS cgs(
     ...
     .SWBRKdet       (               ),
     .OUTBOX         (               ),
     .byte_swap_wr   (byte_swap_wr   ),
     .byte_swap_rd   (byte_swap_rd   )
     );
...
datapool_64kx32 datapool(
    ...
    .DMA1_wrdata  (byte_swap_wr ? DMA1_wrdata_swapped : DMA1wrdata),
...
PDB_1kx32_IRB PDB_IRB0(      //PDB(s) and IRB for shader 0
    ...
    .DMA0_wrdata   (byte_swap_rd ? DMA0_rddata_swapped : DMA0_datapool_rddata),
...
cgsROM #(.ADDRS_WIDTH(12), .DATA_WIDTH(32))
    ...
    .wrdata    (cgs_rom_wren ? cgs_resultout : (byte_swap_rd ? DMA0_rddata_swapped : DMA0_datapool_rddata)),
    .rdenA     (cgs_rdprog        ),
    .rdaddrsA  (cgs_pre_PC        ),
    .rddataA   (cgs_instr         ),
...
shdrROM #(.ADDRS_WIDTH(12), .DATA_WIDTH(32))
    ...
    .wrdata    (cgs_rom0_wren ? cgs_resultout : (byte_swap_rd ? DMA0_rddata_swapped : DMA0_datapool_rddata)),
    .rdenA     (shdr0_rdprog        ),
...
always @(posedge CLKB or posedge RESET) begin
    if (RESET) cgs_sel_q1[1:0] <= 2'b00;
    else cgs_sel_q1[1:0] <= {cgs_rden_DPL, cgs_rden_PDB0};
end

always @(*) begin
    casex(cgs_sel_q1)
         2'bx1 : cgs_rddataA = cgs_PDB_rddata0;
         2'b1x : cgs_rddataA = cgs_datapool_rddata;
        default : cgs_rddataA = 32'h0000_0000;
     endcase   
end

assign DMA1wrdata  = DMA1_PDB_rddata0;
