...
assign nA = wren ? wrdata : 32'h0000_0000;
...
assign inexact = roundit & ~invalid_del;
...
assign raw_excptns = {inexact, underflow, overflow, 1'b0}; //raw exceptions are not yet encloded
                                                      //    v--v--3-bit invalid operation code
//assign NaN_payload = {wraddrsq[4:3], 1'b0, wraddrsq[2:0], 3'b000, 1'b0, pc_q2_del};  // the 1'b0 is reserved for future elongated 13-bit PC vs current 12-bit PC
assign NaN_payload = {3'b000, wraddrsq[2:0], 3'b000, 1'b0, pc_q2_del};  // the 1'b0 is reserved for future elongated 13-bit PC vs current 12-bit PC
                       //      ^-^--thread          ^-^--result buffer location        // the two msb's of wraddrsq is thread number that initiated the invalid operation
                                                             
//                ||--encoded IEEE754 exception is null for "invalid" because these two bits are for back-end use only, which does not signal an back-end exception for invalid when read out 
//                VV                                                               
...
assign invalid_del = delay0[4];
assign wrenq = delay0[3];
assign wraddrsq = delay0[2:0];


    IEEE754_To_FP_filtered ieeetofpA(
      .X (nA),
      .wren (wren),
      .R (nA_FP),
...
//RAM_func #(.ADDRS_WIDTH(5), .DATA_WIDTH(36))
RAM_func #(.ADDRS_WIDTH(3), .DATA_WIDTH(36))
    ram32_ftoi(
    .CLK        (CLK      ),
    .wren       (wrenq    ),
    .wraddrs    (wraddrsq ),   
//                                                    v--if invalid, insert quiet NaN with payload                                                             v--else write result with IEEE exception code, if any
    .wrdata     (invalid_del ? {invalid_del, 1'b0, qNaN} : (qNaN_del[22] ? {2'b00, 2'b00, 1'b0, 8'hFF, 1'b1, qNaN_del[21:0]} : {1'b0, backend_exception[2:0], nR[31:0]})),
