...
assign wrenq = delay2[7];
assign wraddrsq = delay2[6:0];             
...
RAM_tp #(.ADDRS_WIDTH(7), .DATA_WIDTH(34))
    ram128_addclk(
    .CLK        (CLK      ),
    .wren       (wrenq    ),
    .wraddrs    (wraddrsq ),
...
always @(wren or wraddrs or wrdataA or wrdataB or nB) begin
    if (wren) begin
        nA = wrdataA;
        nB = {wrdataB[33:32], (wrdataB[31] ^ wraddrs[4]), wrdataB[30:0]};     //if wraddrs[4] = 1 then subtract
    end
    else begin
        nA = 34'h0_0000_0000;    
        nB = 34'h0_0000_0000; 
