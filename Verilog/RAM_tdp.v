...
module RAM_tdp #(parameter ADDRS_WIDTH = 10, parameter DATA_WIDTH = 32) (
    CLK,
    wrenA,
    wraddrsA,
    wrdataA,
...
wire wren;
wire [(2**ADDRS_WIDTH)-1:0] wraddrs;
wire [DATA_WIDTH-1:0] wrdata;

assign wren = wrenB ? 1'b1 : wrenA;
assign wraddrs = wrenB ? wraddrsB : wraddrsA;
assign wrdata = wrenB ? wrdataB : wrdataA;
...
always @(posedge CLK) begin
    if (rdenA) rddataAq <=  triportRAMA[rdaddrsA];
end
always @(posedge CLK) begin
    if (wren) triportRAMB[wraddrs] <= wrdata;
end
always @(posedge CLK) begin
    if (rdenB) rddataBq <=  triportRAMB[rdaddrsB];  
end
///////////////////////
endmodule    