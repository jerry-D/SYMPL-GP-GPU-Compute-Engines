...
reg [31:0] semaphor;  // one for each memory location  (2 x 16 )  16 for add and 16 for sub
reg readyA;
reg readyB;
...
assign NaN_payload = {1'b0, wraddrsq[4:0], add_oob, 1'b0, pc_q2_del};  // the 1'b0 is reserved for future elongated 13-bit PC vs current 12-bit PC
...
assign AorB_is_Inf_del = delay3[7];
//assign invalid_del = delay3[8];
assign invalid_del = delay3[6];
//assign wrenq = delay3[7];
assign wrenq = delay3[5];
//assign wraddrsq = delay3[6:0];             
assign wraddrsq = delay3[4:0];             
